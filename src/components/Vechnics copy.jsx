...
function Vechnics({ side, audioSettings }) {
  // STATE för interaktiv funktionalitet
  const [isPlaying, setIsPlaying] = useState(false);           // Ljudet spelas/pausas
  const [isPlatterSpinning, setIsPlatterSpinning] = useState(false); // Skivan snurrar
  const [track, setTrack] = useState(null);                    // Aktuellt ljudspår
  const [tonearmAngle, setTonearmAngle] = useState(0);         // Tonarmens rotation
  const [isDragging, setIsDragging] = useState(false);         // Tonarmen dras med mus
  const [startAngle, setStartAngle] = useState(0);
  const [originAngle, setOriginAngle] = useState(0);
  const [isPowered, setIsPowered] = useState(false);           // Spelaren på/av
  const [speed, setSpeed] = useState(null);                    // 33 eller 45 RPM
  const [isLightOn, setIsLightOn] = useState(false);           // Röd lampa tänd
  const [isStrobeOn, setIsStrobeOn] = useState(false);         // Strobe-ljus tänd
  const [pitch, setPitch] = useState(1);                       // Pitchfaktor (1 = normal)
  ...
  // Växlar mellan att spela och pausa ljudet
  const togglePlay = () => {
  ...
  // När användaren laddar upp ett ljudspår
  ...
  // Beräknar vinkeln på muspekaren i förhållande till tonarmens origo
  const getAngle = (e) => {
  ...
  // När användaren börjar dra i tonarmen
  ...
  // När musen släpps, återställ tonarmen
  const handleMouseUp = () => {
    setIsDragging(false);
    setTonearmAngle(0);
  };

  // Uppdatera tonarmens vinkel i realtid
  ...
  // Power-knappen slår på/av och hanterar relaterade tillstånd
  const handlePowerToggle = () => {
    console.log("✅ Power button clicked");
    const newPower = !isPowered;
    setIsPowered(newPower);
    const platter = document.querySelector(`#platter-${side.replaceAll(' ', '-')}`);
      if (!newPower && platter) {
    ...
    <div className="relative inline-block select-none">
      <div className="relative w-[1154px] h-[898px]">
        ...
        <img id={`platter-${side.replaceAll(' ', '-')}`} src={platter} className="absolute top-0 left-0 w-full h-full z-10 pointer-events-none" style={{ transformOrigin: '457px 443px' }} alt="platter" />
                // Vitgul strobe-spot riktad upp från lampan
        {isStrobeOn && (
          <div
            className="absolute z-50 pointer-events-none"
            style={{
              ...
              width: "240px",
              height: "240px",
              background: "radial-gradient(circle at 80% 90%, rgba(255, 255, 200, 0.4), transparent 70%)",
              filter: "blur(12px)",
        ...
        {/* DEBUG ZONER */}
        // Klickzon för powerknapp – aktiverar/avaktiverar spelaren
        ...
        <div
          onClick={() => {
            ...
            platter.style.animation = `spin-up ${rate}s linear infinite forwards`;
              ...
              if (platter) {
              platter.style.animationTimingFunction = 'ease-out';
              platter.style.animationDuration = '2.5s';
              platter.style.animationIterationCount = '1';
              platter.style.animationFillMode = 'forwards';
              platter.style.animationName = 'spin-up';
            }
            }
            return newState;
          });
        ...
        <div onClick={() => setIsStrobeOn(l => !l)} title="Strobe light" className={`absolute z-[96] cursor-pointer bg-pink-500/30 ${!isPowered ? "pointer-events-none opacity-50" : ""}`} style={{ left: "639px", top: "838px", width: "40px", height: "40px" }} />
        ...
        // Interaktiv tonarm med realistisk rotation kring korrekt origo
